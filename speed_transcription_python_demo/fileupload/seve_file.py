...
lfasr_host = 'https://upload-ost-api.xfyun.cn/file'
# 请求的接口名
...
class SeveFile:
    ...
    def upload_cut_complete(self, body_dict):
        ...
        if fileurl and isinstance(fileurl, dict) and 'data' in fileurl and 'url' in fileurl['data']:
            fileurl = fileurl['data']['url']
            print("任务上传结束")
            return fileurl
        else:
            print("[ERROR] 分块上传完成失败，返回：", fileurl)
            return None
