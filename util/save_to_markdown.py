...
def save_to_markdown(url: str, summary: str, output_path: str, model_name: str, tags: Optional[Dict[str, Any]] = None):
    ...
    print(f"[DEBUG] 接收到的标签: {tags}")
    
    if tags:
        content_tags = tags.get("content_tags", [])
        user_purpose = tags.get("user_purpose", [])
        
        print(f"[DEBUG] 内容标签: {content_tags}")
        print(f"[DEBUG] 用户目的: {user_purpose}")
...
"""
    ...
    # 打印标签信息
    if tags:
        print(f"内容标签: {', '.join(tags.get('content_tags', []))}")
        print(f"记录目的: {', '.join(tags.get('user_purpose', []))}") 